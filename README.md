...
Now you can rename `src/App.css` to `src/App.scss` and update `src/HomeComponent.js` to import `src/App.scss`.
...
Import required reactstrap components within `src/HomeComponent.js` file or your custom component files:
...
4. Add `// @flow` to any files you want to type check (for example, to `src/HomeComponent.js`).
...
To try it, delete all the code in `src/HomeComponent.js` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `App.test.js` and `HomeComponent.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
